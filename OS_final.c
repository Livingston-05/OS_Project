...
int main()
{
  ...
  float total_waitingTime = 0, total_turnaroundTime = 0, Avg_waitingTime, Avg_turnaroundTime;
  ...
  else
  {
    ...
    printf("So you'll wake up at %d:00 tommorrow, great!\n", wake_time);
  }
  for (int i = 0; i < 24; ++i)
  {
  ...
  for (t = wake_time; t < 24; t++)
  {
    ...
    for (int i = 0; i < n; i++)
    {
      if (a[highest_p].priority < a[i].priority && a[i].arrivalTime <= t && a[i].burstTime > 0)
      {
        highest_p = i;
  ...
  if(count!=n)
  {
    printf("\nDon't worry as you didn't finish in time, you can do the remaining activities tomorrow, %s! :)\n",name);
  }
  return 0;
  return 0;
}